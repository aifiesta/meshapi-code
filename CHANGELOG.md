...
## 0.5.10 — 2026-08-21 · "smart routing"

Upgrading from 0.5.8? You get this **and** everything listed under 0.5.9 below, which was built and tested but never published separately.

- **Smart routing now covers the whole catalog — 923 models across 16 cohorts.** The bundled routing table is built from *measured* evidence, not vendor metadata: 471 text models probed live for real tool-calling, structured-output, vision and context support, 425 speed-measured, and every one of the 520 media models liveness-checked (22 dead ones excluded outright). This matters because catalog flags are demonstrably wrong — `supports_thinking` reports `False` for opus-4.8, and 47 models publish `context_window` instead of `context_length`, which made a 2M-context model look like it had none. 107 models sit on a cohort frontier and are pickable.
- **Only models that can actually win are pickable.** Each cohort keeps a Pareto frontier over (quality, cost, speed): a model that is worse on *every* axis can't be selected at any weight setting, so the `/route weights` knobs stay honest. A quality floor sits under that, which is what stops "fast and cheap but useless" — a code model with a writing score of 27/100 was being chosen for prose purely because it was quick.
- **Routing reacts to evidence, not just predictions.** Most routers pick once and hope. This one escalates a rung when the model *demonstrates* struggle — placeholder markers in written files, two malformed tool calls in a row, or a stall loop — and settles back down after 3 clean hops, never below the difficulty the turn started at. Escalation is capped at 2 per turn.
- **Per-step routing inside a plan.** A 13-step plan is 13 different tasks. Each `update_step(..., 'in_progress')` re-picks for that step's own cohort and difficulty, so scaffolding steps ride a cheap model while the hard middle step gets a strong one. Stickiness holds between similar steps but is deliberately bypassed when difficulty changes.
- **Switch cost is priced in.** Changing models discards the provider's prompt cache, so the incumbent gets a bonus that scales with how much history is at stake — a marginal scoring win no longer pays a real cache bill.
- **`/effort`** — set routing depth directly with an interactive slider: `auto` (detected per prompt) or a pinned `low … max`. `/route` with no arguments now opens the same slider instead of spending money to show you a status line.
- **`/output` — writing styles.** `concise` (answer first, no lists, no "In summary" closer), `default`, `explanatory` (names the mechanism and at least one explicit trade-off), and `learning` (analogy or worked example, ending in a "Try this:" line). Applies immediately to the running session, and governs prose only — it never shortens the actual work.
- **Unique-prefix slash commands.** `/eff` runs `/effort`, `/ou` runs `/output`. An ambiguous prefix lists the candidates instead of guessing.
- **A one-line "what's new" note** on the first launch after an upgrade, shown once and never on a fresh install.
- Fixed: a short follow-up was treated as a continuation purely by character count, so **"write an essay on LLMs" (22 chars) inherited the previous turn's coding cohort** and was routed to a code model. Continuations are now judged by shape (`2`, `yes`, `continue`) rather than length.
- Fixed: the slider used a fixed 10-column cell, so an 11-character label overflowed it — labels touched, the stops row ran wider than the rail, and the ▲ pointed at the wrong option. Cells now size to their labels and the layout is asserted from 30 to 120 columns.
- Fixed: output styles originally *described* an intent rather than prescribing behaviour, and all three produced the same numbered list with an "Overall…" summary — including "Concise". Each style now names the structures it requires and forbids, verified by a live A/B on the same prompt.
- 797 tests (up from 609).

## 0.5.9 — 2026-08-19 · "the immortal loop" *(never published separately — ships inside 0.5.10)*
- **Smart routing (`/route smart`).** The CLI can now pick a model per prompt *locally* — no classifier tokens billed, no added latency, fully deterministic. Each prompt is classified into a task cohort (coding, agentic, extraction, writing, math, vision, long-context…) and the pick is made from a bundled routing table of measured model scores, steered by your weights: `/route weights cost=0.5 cap=0.3 speed=0.2`. `/route why` shows the cohort, the pick, and the runner-ups with their per-axis scores. Picks stay sticky within a session while the incumbent remains competitive (no mid-conversation model flapping), and the whole layer fails open — if anything is missing or wrong you simply get your pinned model. Gateway auto-routing (`/route auto`) is unchanged and remains available.
